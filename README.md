주짓수 수련생을 위한 라이프스타일 통합 플랫폼입니다.  
오픈매트와 대회 정보를 한곳에서 탐색하고, 참가 흐름과 운영 정보를 더 쉽게 연결하는 서비스를 만들고 있습니다.
...
주짓수 커뮤니티에서는 오픈매트 공지와 대회 정보가 주로 인스타그램 스토리, 단체 채팅방, 개별 게시물처럼 여러 채널에 흩어져 있어 필요한 정보를 제때 찾기 어렵다는 문제가 있었습니다.  
...
단순한 정보 나열이 아니라 실제 참여 행동까지 연결되는 구조를 목표로 했고, 사용자 앱과 관리자 운영 기능을 함께 고려해 확장 가능한 서비스 형태로 설계했습니다.
...
**UI / Utility**  
![Table Calendar](https://img.shields.io/badge/Table_Calendar-0F9D58?style=for-the-badge&logo=googlecalendar&logoColor=white)
![Intl](https://img.shields.io/badge/Intl-607D8B?style=for-the-badge)
![Image Picker](https://img.shields.io/badge/Image_Picker-5C6BC0?style=for-the-badge)
![URL Launcher](https://img.shields.io/badge/URL_Launcher-1976D2?style=for-the-badge)
![Pretendard](https://img.shields.io/badge/Pretendard-F5F5F5?style=for-the-badge&logo=fontforge&logoColor=black)
...
- **인증(Auth)**: 소셜 로그인, 토큰 발급 및 갱신, 로그아웃, 회원 탈퇴와 같은 사용자 인증 흐름을 담당합니다.
- **사용자(User)**: 내 정보 조회 및 수정, FCM 토큰 등록 및 삭제, 사용자 차단 기능을 제공합니다.
- **오픈매트(OpenMat)**: 오픈매트 생성, 수정, 삭제, 목록/상세 조회, 참가 신청 및 취소, 내가 신청한 목록과 내가 개최한 목록 조회 기능을 담당합니다.
...
### 1. 삭제된 오픈매트에 알림으로 진입할 때 상세 조회가 불가능했던 문제
...
### 2. 푸시 알림 수신 여부와 사용자 알림함 상태가 일치하지 않던 문제
...
| 이름 | 역할 |
| --- | --- |
| 정원철 | 1인 개발로 서비스 기획부터 UI 구현, 모바일 앱 개발, 백엔드 API 개발, 데이터베이스 설계, 관리자 기능 및 운영 구조 설계까지 전 과정을 직접 수행 |
